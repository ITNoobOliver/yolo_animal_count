...
class CopyPaste(BaseMixTransform):
    ...
    def __call__(self, labels):
        ...
        # ！！！！ 自己改的！！！！ 检查是否有bounding box属性而不是segment属性！！！！
        if len(labels["instances"].bboxes) == 0 or self.p == 0:
            return labels
        # ！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
    ...
    #
    #         # !!!!!用bounding box直接代替图像分割！！！！ 自己改的！
    #         # bbox = instances2.bboxes[j].astype(np.int32)
    #         # x1, y1, x2, y2 = bbox
    #         # cv2.rectangle(im_new, (int(x1), int(y1)), (int(x2), int(y2)), (1, 1, 1), thickness=cv2.FILLED)
    #
    #         #！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
    #
    #
    ...
    # def _transform(self, labels1, labels2={}):
    #     """Applies Copy-Paste augmentation with rotation to combine objects from another image into the current image."""
    #     im = labels1["img"]
    #     cls = labels1["cls"]
    #     h, w = im.shape[:2]
    #     instances = labels1.pop("instances")
    ...
    #     indexes = indexes[sorted_idx]
    #
    #     result = labels2.get("img", cv2.flip(im, 1))  # 获取源图像
    #
    #     for j in indexes[: round(self.p * n)]:
    #         # 获取边界框坐标
    #         bbox = instances2.bboxes[j].astype(np.int32)
    ...
    #         src_region = result[y1:y2, x1:x2].copy()
    #
    #         # 随机旋转角度和缩放因子
    ...
    #
    #         # 计算旋转矩阵
    #         center = (int(box_width // 2), int(box_height // 2))
    #
    #         # 如果区域太小，跳过旋转，使用原始边界框
    ...
    #
    #             # 创建三通道旋转掩码
    #             rotated_mask = np.zeros((box_height, box_width, 3), dtype=np.uint8)
    #             cv2.rectangle(rotated_mask, (0, 0), (box_width, box_height), (1, 1, 1), thickness=cv2.FILLED)
    ...
    #
    #             # 更新原始掩码
    #             rotated_full_mask = np.zeros_like(im, dtype=np.uint8)
    #             rotated_full_mask[y1:y2, x1:x2] = rotated_mask
    #
    #             # 将旋转的区域放入临时图像
    #             temp_img[y1:y2, x1:x2] = np.where(
    #                 rotated_mask > 0,
    #                 rotated_region,
    #                 temp_img[y1:y2, x1:x2]
    #             )
    #
    #             # 更新掩码
    #             im_new = np.logical_or(im_new, rotated_full_mask).astype(np.uint8)
    #
    #             # 更新类别信息
    #             cls = np.concatenate((cls, labels2.get("cls", cls)[[j]]), axis=0)
    #
    ...
    def _transform(self, labels1, labels2={}):
        """
        Applies Copy-Paste augmentation with 90-degree rotations to combine objects from another image.
        Rotates both the bounding boxes and the actual image content.
        """
        import cv2
        import random
        import numpy as np
        ...
        # Get target image, class labels and dimensions
        im = labels1["img"]
        cls = labels1["cls"]
        h, w = im.shape[:2]

        # 创建一个副本用于最终展示（包含原图 + 粘贴内容 + 只有粘贴内容的边界框）
        final_visualization = im.copy()  # 添加这一行来创建原始图像的副本

        # 创建一个列表来存储粘贴对象的信息
        pasted_objects = []  # 添加这一行来初始化粘贴对象信息列表

        # Extract and prepare instances from target image
        instances = labels1.pop("instances")
        instances.convert_bbox(format="xyxy")
        instances.denormalize(w, h)

        # Create mask for pasting
        im_new = np.zeros(im.shape, np.uint8)

        # Get instances from source image or create by flipping target
        instances2 = labels2.pop("instances", None)
        if instances2 is None:
            instances2 = deepcopy(instances)
            instances2.fliplr(w)

        # Get source image or create by flipping target image if not provided
        source_img = labels2.get("img", cv2.flip(im, 1))
        ...
        if n == 0:  # 修复：如果没有合适的索引，直接返回
            labels1["img"] = im
            labels1["cls"] = cls
            labels1["instances"] = instances
            return labels1

        # Sort by IoA and select objects to paste
        sorted_idx = np.argsort(ioa.max(1)[indexes])
        indexes = indexes[sorted_idx]

        # Create a canvas for rotated objects (same size as target image)
        rotated_canvas = np.zeros_like(im)

        def sheep_segmentation_mask(patch):
            # 1. HSV分离亮度 + 白色掩码
            ...
            # 3. 亮度 + 白色双重感知掩码
            combined_mask = cv2.bitwise_and(adaptive, color_mask)

            # 4. 距离变换提取前景中心
            dist = cv2.distanceTransform(combined_mask, cv2.DIST_L2, 5)
            _, sure_fg = cv2.threshold(dist, 0.2 * dist.max(), 255, 0)
            sure_fg = sure_fg.astype(np.uint8)

            # 5. 膨胀作为背景（但只做一次，避免覆盖整张图）
            kernel = np.ones((3, 3), np.uint8)
            sure_bg = cv2.dilate(combined_mask, kernel, iterations=1)
            ...
            markers = cv2.watershed(patch_copy, markers)

            # 9. 生成羊掩码（仅分为前景=羊，背景=草）
            mask = np.zeros_like(v, dtype=np.uint8)
            mask[markers > 1] = 1  # 1 表示羊区域

            return mask

        # For each selected instance to paste
        for j in indexes[: round(self.p * n)]:
            # Get the original bbox in [x1, y1, x2, y2] format
            bbox = instances2.bboxes[j].copy()
            x1, y1, x2, y2 = bbox

            # Convert to integer coordinates and ensure they're within image bounds
            x1, y1, x2, y2 = map(int,
                                 [max(0, x1), max(0, y1), min(source_img.shape[1], x2), min(source_img.shape[0], y2)])
            ...
            # Apply random rotation to both the object and its bounding box
            ...
            if rotation_angle == 90:
                ...
                obj = cv2.rotate(obj, cv2.ROTATE_180)
            elif rotation_angle == 270:
                obj = cv2.rotate(obj, cv2.ROTATE_90_COUNTERCLOCKWISE)

            # After rotation, update object dimensions
            obj_h, obj_w = obj.shape[:2]

            # For non-zero rotation, calculate rotated bounding box
            if rotation_angle != 0:
                ...
                # Convert rotation angle to radians (逆时针旋转)
                ...
                # Get the new bounding box coordinates from rotated corners
                new_x1, new_y1 = np.min(rotated_corners, axis=0)
                new_x2, new_y2 = np.max(rotated_corners, axis=0)
            ...
            # Choose a random position to paste within image boundaries
            max_paste_x = max(0, w - obj_w)
            max_paste_y = max(0, h - obj_h)
            ...
            if max_paste_x > 0 and max_paste_y > 0:
                ...
                # 使用新的羊群分割函数
                obj_mask = sheep_segmentation_mask(obj)
                ...
                pasted_objects.append({
                    ...
                    "cls_id": int(labels2.get("cls", cls)[j]) if j < len(labels2.get("cls", cls)) else -1
                })

                # # Create a simple mask for the object (non-black pixels)
                # obj_mask = np.sum(obj, axis=2) > 10
                # obj_mask = obj_mask.astype(np.uint8)

                # Calculate region for pasting
                y_end = min(paste_y + obj_h, h)
                x_end = min(paste_x + obj_w, w)
                obj_h_clip = y_end - paste_y
                obj_w_clip = x_end - paste_x
                #
                # # 将旋转后的对象粘贴到画布上 - 取消注释此段代码
                # for c in range(min(3, im.shape[2])):
                #     target_region = rotated_canvas[paste_y:y_end, paste_x:x_end, c]
                #     source_region = obj[:obj_h_clip, :obj_w_clip, c]
                #     mask_region = obj_mask[:obj_h_clip, :obj_w_clip]
                
                #     # Update only where mask is non-zero
                #     target_region[mask_region > 0] = source_region[mask_region > 0]

                # # 羽化掩膜创建
                blur_kernel = (3, 3)  # 可调参数，越大羽化范围越广
                sigma = 1.5  # 可调参数，值越大模糊越强

                # # 确保obj_mask尺寸正确
                mask_for_blur = obj_mask[:obj_h_clip, :obj_w_clip]
                soft_mask = cv2.GaussianBlur(mask_for_blur.astype(np.float32), blur_kernel, sigma)

                # alpha通道归一化
                alpha = np.clip(soft_mask, 0, 1)[..., np.newaxis]

                # 获取正确的区域进行混合
                target_region = rotated_canvas[paste_y:y_end, paste_x:x_end].astype(np.float32)
                source_region = obj[:obj_h_clip, :obj_w_clip].astype(np.float32)

                # 确保维度匹配
                if alpha.shape[:2] == source_region.shape[:2] and alpha.shape[:2] == target_region.shape[:2]:
                    # alpha混合
                    blended_region = (alpha * source_region + (1 - alpha) * target_region).astype(np.uint8)
                    # 混合区域贴回rotated_canvas
                    rotated_canvas[paste_y:y_end, paste_x:x_end] = blended_region

                # Create a mask for pasting in the final image
                mask_region = np.zeros((h, w), dtype=np.uint8)
                mask_region[paste_y:y_end, paste_x:x_end] = obj_mask[:obj_h_clip, :obj_w_clip]

                # Add this mask to the global mask
                im_new[:, :, 0] = np.maximum(im_new[:, :, 0], mask_region)
                im_new[:, :, 1] = np.maximum(im_new[:, :, 1], mask_region)
                im_new[:, :, 2] = np.maximum(im_new[:, :, 2], mask_region)

                # Update the bounding box to match the paste location
                # 使用new_w和new_h与原始代码保持一致
                new_bbox = np.array([
                    paste_x,
                    paste_y,
                    paste_x + new_w,
                ...
                # Create a copy of the instance to manipulate
                instance_to_add = deepcopy(instances2[j:j + 1])

                # Update the instance's bounding box
                instance_to_add.bboxes[0] = new_bbox

                # 只有在这里添加类别和实例，确保它们同步添加
                if j < len(labels2.get("cls", cls)):  # 修复: 检查索引是否有效
                    cls = np.concatenate((cls, labels2.get("cls", cls)[[j]]), axis=0)
                    instances = Instances.concatenate((instances, instance_to_add), axis=0)

        # Apply copy-paste operation using the mask and the rotated objects
        mask = im_new.astype(bool)
        im[mask] = rotated_canvas[mask]

        # # 将粘贴操作后的结果更新到final_visualization
        # final_visualization[mask] = rotated_canvas[mask]

        # # 只在final_visualization上绘制粘贴对象的边界框
        # for obj in pasted_objects:
        #     x1, y1, x2, y2 = obj["bbox"]
        #     cls_id = obj["cls_id"]

        #     # 绘制边界框
        #     cv2.rectangle(final_visualization,
        #                   (int(x1), int(y1)),
        #                   (int(x2), int(y2)),
        #                   (0, 255, 0), 2)  # 绿色边界框

        #     # 绘制标签
        #     label = f"Class {cls_id} (pasted)"
        #     # 标签背景
        ...
        #                   (0, 255, 0), -1)  # 填充的绿色背景
        #     # 标签文本
        #     cv2.putText(final_visualization,
        #                 label,
        #                 (int(x1), int(y1 - 5)),
        ...
        # # 创建输出目录
        # output_dir = "paste_visualizations"
        # os.makedirs(output_dir, exist_ok=True)

        # # 生成唯一的时间戳文件名
        # timestamp = int(time.time() * 1000)
        # file_path = f"{output_dir}/paste_highlighted_{timestamp}.jpg"

        # # 保存结果
        # cv2.imwrite(file_path, final_visualization)

        # Restore labels structure
        labels1["img"] = im
        labels1["cls"] = cls
        labels1["instances"] = instances
